...
void initialize_dir(int pid)
{
int j = 0,frno = 0;	
pd_t *pdir_base;
get_frm(&frno);
proctab[pid].pdbr = (frno + FRAME0) * NBPG;
frm_tab[frno] = (fr_map_t){FRM_MAPPED, pid, frm_tab[frno].fr_vpno, frm_tab[frno].fr_refcnt, FR_DIR, 0, 0, -1};
pdir_base = (FRAME0+ frno) * NBPG;
int i = 0;
while(j < 1024){
	pdir_base[j].pd_write = 1;
	if (i > 3) {
		break;
	}
	pdir_base[i].pd_pres = 1;
	pdir_base[i].pd_base = FRAME0 + i;
	j++;
	i++;
}
}