...
int queue_head = -1;	/* initializing the value of head of queue */
...
while(i < 4){
	get_frm(&frno);
	frm_tab[frno] = (fr_map_t){FRM_MAPPED, NULLPROC, frm_tab[frno].fr_vpno, frm_tab[frno].fr_refcnt, FR_TBL, 0, 0, -1};
	page_entry = (FRAME0 + frno) * NBPG;
	bs_map++;
	j = 0;
	while(j < 1024){
		(*page_entry).pt_pres = (*page_entry).pt_write = (*page_entry).pt_global = 1;	
	    	(*page_entry).pt_user = (*page_entry).pt_pwt = (*page_entry).pt_pcd = (*page_entry).pt_dirty = (*page_entry).pt_mbz = (*page_entry).pt_avail = 0;
